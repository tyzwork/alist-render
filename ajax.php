...
case 'pre_upload':
	...
	$storage = isset($_POST['storage'])?trim($_POST['storage']):'local';
	...
	// 根据用户选择的存储类型加载对应的存储驱动
	$upload_stor = \lib\StorHelper::getModel($storage);
	if(!$upload_stor){
		exit('{"code":-1,"msg":"存储驱动加载失败"}');
	}

	if(\lib\StorHelper::is_cloud_by_type($storage) && $conf['uploadfile_type'] == 1){
		$param = $upload_stor->getUploadParam($hash, $name, $limit_size * 1024 * 1024);
		if(!$param)exit('{"code":-1,"msg":"获取上传参数失败","errmsg":"'.$upload_stor->errmsg().'"}');
		$_SESSION['upload'] = [
			'chunks' => 1,
			'name' => $name,
			'hash' => $hash,
...
case 'upload_part':
	...
	$storage = $_SESSION['upload']['storage'];
	$upload_stor = \lib\StorHelper::getModel($storage);
	if(!$upload_stor){
		exit('{"code":-1,"msg":"存储驱动加载失败"}');
	}
	...
	if($chunks > 1){
		...
		if($chunks == $chunk){
			...
			$result = $upload_stor->savefile($hash, $savePathTemp, minetype($ext));
			if(!$result)exit('{"code":-1,"msg":"文件上传失败","error":"stor","errmsg":"'.$upload_stor->errmsg().'"}');
		}else{
		...
		$result = $upload_stor->upload($hash, $_FILES['file']['tmp_name'], minetype($ext));
		if(!$result)exit('{"code":-1,"msg":"文件上传失败","error":"stor","errmsg":"'.$upload_stor->errmsg().'"}');
	}
	...
	$sds = $DB->exec("INSERT INTO `pre_file` (`name`,`type`,`size`,`hash`,`addtime`,`ip`,`hide`,`pwd`,`uid`,`storage`) values (:name,:type,:size,:hash,NOW(),:ip,:hide,:pwd,:uid,:storage)", [':name'=>$name, ':type'=>$ext, ':size'=>$size, ':hash'=>$hash, ':ip'=>$clientip, ':hide'=>$hide, ':pwd'=>$pwd, ':uid'=>($uid?$uid:0), ':storage'=>$storage]);
...
case 'complete_upload':
	...
	if(!$upload_stor->exists($hash)){
		exit('{"code":-1,"msg":"文件上传失败","error":"stor","errmsg":"'.$upload_stor->errmsg().'"}');
	}
...
case 'deleteFile':
	...
	$result = $file_stor->delete($row['hash']);
