...
<select class="form-control" id="storage" v-model="input.storage" style="max-width:220px;">
<option value="local">本地存储</option>
<?php
$storages = ['oss' => '阿里云OSS', 'qcloud' => '腾讯云COS', 'obs' => '华为云OBS', 'upyun' => '又拍云', 'qiniu' => '七牛云', 's3' => 'AWS S3兼容存储', 'webdav' => 'WebDAV'];
foreach($storages as $k => $v) {
    if(!empty($conf[$k.'_ak']) || !empty($conf[$k.'_url']) || !empty($conf[$k.'_bucket'])) {
